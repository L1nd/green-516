	...
	if (((get_dist(src, usr) <= 1) && istype(src.loc, /turf)))
		...
		if (href_list["relabel"])
			if (can_label)
				...
				var/label = input("Choose canister label", "Gas canister") as null|anything in can_colors
				if (label)
					src.can_color = can_colors[label]
					src.icon_state = can_colors[label]
