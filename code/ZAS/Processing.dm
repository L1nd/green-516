...
proc/ShareSpace(datum/gas_mixture/A, ratio)
	...
	var/space_heat_capacity = MINIMUM_HEAT_CAPACITY * share_size

	var/oxy_avg = (full_oxy) / (size + share_size)
	var/nit_avg = (full_nitro) / (size + share_size)
	var/co2_avg = (full_co2) / (size + share_size)
	var/plasma_avg = (full_plasma) / (size + share_size)

	var/temp_avg = (A.temperature * full_heat_capacity + TCMB * space_heat_capacity) / (full_heat_capacity + space_heat_capacity)
